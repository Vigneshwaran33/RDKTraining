...
#define SETTINGS_FILE_NAME              "/opt/user_info.conf"
#define SETTINGS_FILE_KEY               "user_name"
...
namespace WPEFramework {
    ...
    namespace Plugin {
        ...
        {
            LOGINFO("ctor");
            UserInfo::_instance = this;
            Register("getUserName", &UserInfo::getUserName, this);
            Register("setUserName", &UserInfo::setUserName, this);
        }
        ...
        void UserInfo::Deinitialize(PluginHost::IShell* /* service */)
        {
            ...
            UserInfo::_instance = nullptr;
        }

        //Begin methods
        uint32_t UserInfo::getUserName(const JsonObject& parameters, JsonObject& response)
        {
            LOGINFOMETHOD();
        ...
        uint32_t UserInfo::setUserName(const JsonObject& parameters, JsonObject& response)
        {
            LOGINFOMETHOD();
            returnIfStringParamNotFound(parameters, SETTINGS_FILE_KEY);
            string name = parameters[SETTINGS_FILE_KEY].String();

            g_autoptr(GKeyFile) file = g_key_file_new();
            g_key_file_set_string(file, SETTINGS_FILE_GROUP, SETTINGS_FILE_KEY, (gchar *)name.c_str());
